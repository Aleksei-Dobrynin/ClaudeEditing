...
const PageGrid = observer((props: GridProps) => {
  ...
  const actions: GridColDef[] = [{
    ...
    getActions: ({ id }) => {
      ...
      if (!props.hideEditButton) {
        actions.push(
          <GridActionsCellItem
            ...
            onClick={() => props.customEditClick ? props.customEditClick(id) : navigate(`/user/${props.tableName}/addedit?id=${id}`)}
            color="inherit"
          />)
      }
      ...
      if (props.customActionButton) {
        actions.push(props.customActionButton(id as number));
      }
  ...
  return (
    <Paper elevation={5} style={{ width: '100%', padding: 20, marginBottom: 30 }}>
      {!props.hideTitle && <h1 data-testid={`${props.tableName}HeaderTitle`}>{props.title}</h1>}
      <Box
        sx={props.isInLineHeader ? {
          display: 'flex',
          alignItems: 'center',
