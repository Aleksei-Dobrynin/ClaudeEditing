...
import storeList from "./../application_legal_recordListView/store";
...
const FastInputView: FC<application_legal_recordProps> = observer((props) => {
  ...
  useEffect(() => {
    if (props.idMain !== 0 && storeList.idMain !== props.idMain) {
      ...
      storeList.loadapplication_legal_records();
    }
  }, [props.idMain]);

  const columns = [
    {
                    field: 'id_applicationNavName',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_legal_recordListView.id_application"),
                },
                {
                    field: 'id_legalrecordNavName',
                    width: null, //or number from 1 to 12
                    headerName: translate("label:application_legal_recordListView.id_legalrecord"),
                },
                {
                    field: 'id_legalactNavName',
                    width: null, //or number from 1 to 12
  ...
  return (
    <Container>
      <Card component={Paper} elevation={5}>
        <CardContent>
          <Box id="application_legal_record_TitleName" sx={{ m: 1 }}>
            <h3>{translate("label:application_legal_recordAddEditView.entityTitle")}</h3>
          </Box>
          <Divider />
                    ...
                    {storeList.isEdit === false && (
                      <>
                        ...
                        <IconButton
                          ...
                          onClick={() => storeList.deleteapplication_legal_record(entity.id)}
                        >
                          <DeleteIcon />
                        </IconButton>
                      </>
          ...
          {storeList.isEdit ? (
            ...
            <Grid item display={"flex"} justifyContent={"flex-end"} sx={{ mt: 2 }}>
              <CustomButton
                variant="contained"
                size="small"
                id="id_application_legal_recordAddButton"
                onClick={() => {
                  storeList.setFastInputIsEdit(true);
                  store.doLoad(0);
