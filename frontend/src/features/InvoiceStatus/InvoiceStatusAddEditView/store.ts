...
import { getInvoiceStatus, createInvoiceStatus, updateInvoiceStatus } from "api/InvoiceStatus";
...
class NewStore {
    ...
    if (canSave) {
      try {
        ...
        const response = data.id === 0
          ? await createInvoiceStatus(data)
  ...
  loadInvoiceStatus = async (id: number) => {
    try {
      MainStore.changeLoader(true);
