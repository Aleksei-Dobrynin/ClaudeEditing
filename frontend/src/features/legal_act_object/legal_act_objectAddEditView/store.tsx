...
import { getlegal_act_object } from "api/legal_act_object";
import { createlegal_act_object } from "api/legal_act_object";
import { updatelegal_act_object } from "api/legal_act_object";
...
class NewStore {
  ...
  async onSaveClick(onSaved: (id: number) => void) {
    ...
    try {
      ...
      let response;
      if (this.id === 0) {
        response = await createlegal_act_object(data);
      } else {
        response = await updatelegal_act_object(data);
      }
  ...
  async doLoad(id: number) {
    ...
    this.loadlegal_act_object(id);
  }

  loadlegal_act_object = async (id: number) => {
    try {
      MainStore.changeLoader(true);
      const response = await getlegal_act_object(id);
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
          ...
          this.id_act = response.data.id_act;
