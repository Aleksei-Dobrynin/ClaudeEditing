...
import {getEmployeeDashboardInfo} from "api/Employee/useGetEmployeeByEmail";
// dictionaries
...
class NewStore {
  ...
  finance_date_start = dayjs().add(-6, 'month')
  ...
  appCount_date_start = dayjs().add(-6, 'month')
  ...
  financeInvoice_date_start = dayjs().add(-6, 'month')
  ...
  archive_count_date_end = dayjs()
  archive_Counts: number[] = []
  archive_Status: string[] = []

  ApplicationsCategoryCount: any[] = [];
  category_count_date_start = dayjs().add(-1, 'year')
  ...
  async doLoad() {
    this.loadEmployeeDashboardInfo();
    // this.loadStructures();
    this.loadDistrics();
  ...
  async loadEmployeeDashboardInfo() {
    try {
      MainStore.changeLoader(true);
      const response = await getEmployeeDashboardInfo();
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
          this.Structures = response.data.dashboard_structures;
  ...
  loadArchiveCountWeek = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getDashboardGetArchiveCount(this.archive_count_date_start?.toISOString(), this.archive_count_date_end?.add(1, 'day').toISOString());
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
