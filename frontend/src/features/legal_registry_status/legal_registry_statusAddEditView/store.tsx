...
import { getlegal_registry_status } from "api/legal_registry_status";
import { createlegal_registry_status } from "api/legal_registry_status";
import { updatelegal_registry_status } from "api/legal_registry_status";
...
class NewStore {
  ...
  async onSaveClick(onSaved: (id: number) => void) {
    ...
    try {
      ...
      let response;
      if (this.id === 0) {
        response = await createlegal_registry_status(data);
      } else {
        response = await updatelegal_registry_status(data);
      }
  ...
  async doLoad(id: number) {
    ...
    this.loadlegal_registry_status(id);
  }

  loadlegal_registry_status = async (id: number) => {
    try {
      MainStore.changeLoader(true);
      const response = await getlegal_registry_status(id);
