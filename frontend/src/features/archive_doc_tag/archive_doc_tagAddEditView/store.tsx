...
import { getarchive_doc_tag } from "api/archive_doc_tag";
import { createarchive_doc_tag } from "api/archive_doc_tag";
import { updatearchive_doc_tag } from "api/archive_doc_tag";
...
class NewStore {
  ...
  async onSaveClick(onSaved: (id: number) => void) {
    ...
    try {
      ...
      let response;
      if (this.id === 0) {
        response = await createarchive_doc_tag(data);
      } else {
        response = await updatearchive_doc_tag(data);
      }
  ...
  async doLoad(id: number) {
    ...
    this.loadarchive_doc_tag(id);
  }

  loadarchive_doc_tag = async (id: number) => {
    try {
      MainStore.changeLoader(true);
      const response = await getarchive_doc_tag(id);
