...
import {
  getDashboardGetRefucalCountByStructure
} from "api/Dashboard";
  ...
  loadIssuedAppsRegister = async () => {
    if (this.date_start?.isValid() && this.date_end?.isValid()) {
      try {
        ...
        const response = await getDashboardGetRefucalCountByStructure(getFormattedDateToDashboard(this.date_start), getFormattedDateToDashboard(this.date_end));
