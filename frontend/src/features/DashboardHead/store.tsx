...
import {
  getDashboardGetApplicationCountWeek,
  getDashboardGetApplicationCountHour,
  ...
  getForFinanceInvoice,
  getloadApplicationsCategoryCount,
  getDashboardGetAppsByStatusAndStructure,
  getloadApplicationsCategoryCountForMyStructure,
  getDashboardGetCountObjectsMyStructure,
  getDashboardGetCountUserApplicationsMyStructure,
  getDashboardGetApplicationCountHourMyStructure,
  getDashboardGetApplicationCountWeekMyStructure,
  getDashboardAppCountMyStructure,
  getDashboardGetArchiveCountMyStructure,
  getDashboardGetCountApplciationsForMyStructure, getDashboardGetCountTasksForMyStructure
} from "api/Dashboard";
import { getStructures } from "api/Structure/useGetStructures";
import { getDistricts } from "api/District/useGetDistricts";
import { getMyStructure, getServices } from "api/Service/useGetServices";
...
class NewStore {
  ...
  pie_date_end = dayjs()
  pie_structure_id = LayoutStore.head_of_structures[0]?.id ?? 0;
  pie_selected_status = "";
  pieEmployees: string[] = []
  ...
  task_structure_id = LayoutStore.head_of_structures[0]?.id ?? 0;
  ...
  loadApplicationsDashboard = async () => {
    try {
      ...
      const response = await getDashboardGetCountApplciationsForMyStructure(this.pie_date_start, this.pie_date_end?.add(1, 'day'));
  ...
  loadAppCountDashboard = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getDashboardAppCountMyStructure(this.appCount_date_start, this.appCount_date_end?.add(1, 'day'), this.appCount_service_id, this.appCount_status_id);
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
  ...
  loadTasksDashboard = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getDashboardGetCountTasksForMyStructure(this.task_date_start, this.task_date_end?.add(1, 'day'));
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
  ...
  loadUserApplicationsDashboard = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getDashboardGetCountUserApplicationsMyStructure(this.task_date_start, this.task_date_end?.add(1, 'day'),);
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
  ...
  loadApplicationCountHour = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getDashboardGetApplicationCountHourMyStructure(this.app_count_hour_appcount_date_start, this.app_count_hour_appcount_date_end?.add(1, 'day'));
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
  ...
  loadApplicationCountWeek = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getDashboardGetApplicationCountWeekMyStructure(this.app_count_week_count_date_start, this.app_count_week_count_date_end?.add(1, 'day'));
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
  ...
  loadArchiveCountWeek = async () => {
    if (this.archive_count_date_start?.isValid() && this.archive_count_date_end?.isValid()) {
      try {
        MainStore.changeLoader(true);
        const response = await getDashboardGetArchiveCountMyStructure(getFormattedDateToDashboard(this.archive_count_date_start), getFormattedDateToDashboard(this.archive_count_date_end));
        if ((response.status === 201 || response.status === 200) && response?.data !== null) {
          runInAction(() => {
  ...
  loadApplicationsCategoryCount = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getloadApplicationsCategoryCountForMyStructure(
  ...
  loadHeadStructures = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getDashboardInfo();
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
          this.headStructures = response.data.dashboard_head_of_structures;
        });
      } else {
        throw new Error();
      }
  ...
  loadObjectsDashboard = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getDashboardGetCountObjectsMyStructure(this.obj_district_id);
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
  ...
  loadServices = async () => {
    try {
      MainStore.changeLoader(true);
      const response = await getMyStructure();
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {
