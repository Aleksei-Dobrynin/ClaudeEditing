...
import {
  getDashboardEmployeeToDutyPlan
} from "api/Dashboard";
...
class NewStore {
  ...
  loadEmployeesToDutyPlan = async () => {
    if (this.date_start?.isValid() && this.date_end?.isValid()) {
      try {
        MainStore.changeLoader(true);
        const response = await getDashboardEmployeeToDutyPlan(getFormattedDateToDashboard(this.date_start), getFormattedDateToDashboard(this.date_end));
