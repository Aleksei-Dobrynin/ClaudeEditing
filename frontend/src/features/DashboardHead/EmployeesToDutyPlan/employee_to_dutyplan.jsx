...
const EmployeesToDutyPlanDashboard = observer(() => {
  ...
  const columns = [
      {
        ...
        renderCell: (row) => {
          ...
          return <span><a onClick={() => {
            // const json = `{}`
            const json = `{"pin": "", "number": "", "tag_id": 0, "address": "", "sort_by": null, "date_end": null, "pageSize": 100, "employee_arch_id": 0, "isExpired": false, "sort_type": null, "useCommon": false, "date_start": null, "pageNumber": 0, "status_ids": [], "district_id": 0, "employee_id": 0, "service_ids": [], "customerName": "", "deadline_day": 0, "common_filter": "", "structure_ids": [], "isMyOrgApplication": false, "withoutAssignedEmployee": false}`
            let filterData = JSON.parse(json);
            let filter = storeApplication.filter;
            filterData.employee_arch_id = row.employee_id;
    ...
    <Paper elevation={2} sx={{ p: 2, height: '100%' }}>
      <Grid container spacing={2}>
        <Grid item xs={12} md={12} display={"flex"} justifyContent={"center"}>
          <h2>
            {translate("Переданные заявки в ОЦиДП")}
          </h2>
        </Grid>
...
export default EmployeesToDutyPlanDashboard