...
import {
  getDashboardGetCountBySelectedStructure
} from "api/Dashboard";
...
class NewStore {
  ...
  loadIssuedAppsRegister = async () => {
    if (dashboardStore.selected_count_structure_id == 0) return;
    if (this.date_start?.isValid() && this.date_end?.isValid()) {
      try {
        MainStore.changeLoader(true);
        const response = await getDashboardGetCountBySelectedStructure(getFormattedDateToDashboard(this.date_start), getFormattedDateToDashboard(this.date_end), dashboardStore.selected_count_structure_id);
