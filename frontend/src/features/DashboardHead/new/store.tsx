...
import {
  getDashboardGetCountTaskByStructure
} from "api/Dashboard";
...
import dashboard from "../../DashboardHead";
  ...
  loadIssuedAppsRegister = async () => {
    if (this.date_start?.isValid() && this.date_end?.isValid()) {
      try {
        ...
        const response = await getDashboardGetCountTaskByStructure(getFormattedDateToDashboard(this.date_start), getFormattedDateToDashboard(this.date_end));
        if ((response.status === 201 || response.status === 200) && response?.data !== null) {
          runInAction(() => {
            ...
            this.data = response.data.filter(x => dashboardStore.headStructures.some(z => z.id === x.structure_id))
          });
        } else {
          throw new Error();
        }
