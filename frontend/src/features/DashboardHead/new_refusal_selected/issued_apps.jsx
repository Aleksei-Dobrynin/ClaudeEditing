...
const RefusalCountBySelectedStructureDashboard = observer(() => {
  ...
  const columns = [
    {
      ...
      renderCell: (row) => {
        ...
        return <span><a onClick={() => {
          ...
          filterData.status_ids = [3, 5, 19, 9]; //TODO
  ...
  return (
    <Paper elevation={2} sx={{ p: 2, height: '100%' }}>
      <Grid container spacing={2}>
        <Button onClick={()=>{
          dashboardStore.selected_refusal_count_structure_id = 0;
          dashboardStore.selected_refusal_count_structure_name = '';
        }}>Назад</Button>
        <Grid item xs={12} md={12} display={"flex"} justifyContent={"center"}>
          <h2>
            {translate("Отказанные заявки: ") + dashboardStore.selected_refusal_count_structure_name}
          </h2>
        </Grid>
...
export default RefusalCountBySelectedStructureDashboard