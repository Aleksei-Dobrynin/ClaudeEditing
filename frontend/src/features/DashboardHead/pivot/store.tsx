...
import { getApplicationsWithCoords, getForPivotDashboard, getForPivotDashboardMyStructure } from "api/Dashboard";
  ...
  loadApplications = async () => {
    if (this.pivot_date_start?.isValid() && this.pivot_date_end?.isValid()) {
      try {
        ...
        const response = await getForPivotDashboardMyStructure(this.pivot_date_start, this.pivot_date_end?.add(1, 'day'), this.service_id, this.status_id);
