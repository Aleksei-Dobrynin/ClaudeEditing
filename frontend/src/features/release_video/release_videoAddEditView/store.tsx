...
import { getrelease_video } from "api/release_video";
import { createrelease_video } from "api/release_video";
import { updaterelease_video } from "api/release_video";
...
class NewStore {
  ...
  async onSaveClick(onSaved: (id: number) => void) {
    ...
    try {
      ...
      let response;
      if (this.id === 0) {
        response = await createrelease_video(data);
      } else {
        response = await updaterelease_video(data);
      }
  ...
  async doLoad(id: number) {
    ...
    this.loadrelease_video(id);
  }

  loadrelease_video = async (id: number) => {
    try {
      MainStore.changeLoader(true);
      const response = await getrelease_video(id);
