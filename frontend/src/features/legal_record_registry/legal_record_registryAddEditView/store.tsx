...
import { getlegal_record_registry } from "api/legal_record_registry";
import { createlegal_record_registry } from "api/legal_record_registry";
import { updatelegal_record_registry } from "api/legal_record_registry";
...
class NewStore {
  ...
  async onSaveClick(onSaved: (id: number) => void) {
    ...
    try {
      ...
      let response;
      if (this.id === 0) {
        response = await createlegal_record_registry(data);
      } else {
        response = await updatelegal_record_registry(data);
      }
  ...
  async doLoad(id: number) {
    ...
    await this.loadlegal_record_registry(id);
  }

  loadlegal_record_registry = async (id: number) => {
    try {
      MainStore.changeLoader(true);
      const response = await getlegal_record_registry(id);
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
        runInAction(() => {

          this.id = response.data.id;
          this.is_active = response.data.is_active;
          this.defendant = response.data.defendant;
          this.id_status = response.data.id_status;
          this.subject = response.data.subject;
          this.complainant = response.data.complainant;
          ...
          this.assignees = this.employeeInStructure.filter(x =>
            this.assigneesIds.includes(x.id)
          );
        });
      } else {
        throw new Error();
