...
import { getStreetType, createStreetType, updateStreetType } from "api/StreetType";
...
class NewStore {
  ...
  onSaveClick = async (onSaved: (id: number) => void) => {
    ...
    if (canSave) {
      try {
        ...
        const response = data.id === 0 ? await createStreetType(data) : await updateStreetType(data);
  ...
  loadStreetType = async (id: number) => {
    try {
      MainStore.changeLoader(true);
      const response = await getStreetType(id);
