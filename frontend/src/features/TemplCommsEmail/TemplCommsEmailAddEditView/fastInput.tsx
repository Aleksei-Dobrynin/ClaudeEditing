...
import storeList from './../TemplCommsEmailListView/store'
import CreateIcon from '@mui/icons-material/Create';
import DeleteIcon from '@mui/icons-material/Delete';
import CustomButton from "components/Button";

type TemplCommsEmailProps = {
...
const TemplCommsEmailFastInputView: FC<TemplCommsEmailProps> = observer((props) => {
  ...
  useEffect(() => {
    if (props.idMain !== 0 && storeList.template_comms_id !== props.idMain) {
      ...
      storeList.loadTemplCommsEmails()
    }
  }, [props.idMain])
  ...
  return (
    <Container >
      <Card component={Paper} elevation={5}>
        <CardContent>
          <Box id="TemplCommsEmail_TitleName" sx={{ m: 1 }}>
            <h3>{translate('label:TemplCommsEmailAddEditView.entityTitle')}</h3>
          </Box>
          <Divider />
          <Grid
            container
                ...
                <Grid
                  ...
                  {storeList.isEdit === false && <>
                    ...
                    <IconButton
                      ...
                      onClick={() => storeList.deleteTemplCommsEmail(entity.id)}>
                      <DeleteIcon />
                    </IconButton>
          ...
          {storeList.isEdit ? <Grid container spacing={3} sx={{ mt: 2 }}>
            ...
            <Grid item display={"flex"} justifyContent={"flex-end"} sx={{ mt: 2 }}>
              <CustomButton
                variant="contained"
                size="small"
                id="id_TemplCommsEmailAddButton"
                onClick={() => {
                  storeList.setFastInputIsEdit(true)
                  store.doLoad(0)
...
export default TemplCommsEmailFastInputView;
