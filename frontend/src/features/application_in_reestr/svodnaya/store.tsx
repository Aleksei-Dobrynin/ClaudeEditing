...
import { getSvodnaya } from "api/application_in_reestr";
import { getapplication_in_reestrsByreestr_id } from "api/application_in_reestr";
import { changeReestrStatus } from "api/reestr";
...
class NewStore {
  ...
  async loadapplication_in_reestrs() {
    try {
      ...
      const response = await getSvodnaya(this.year, this.month, status_code);
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
