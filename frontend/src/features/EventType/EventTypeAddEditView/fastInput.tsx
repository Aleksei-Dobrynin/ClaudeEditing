...
import storeList from "./../EventTypeListView/store";
...
const FastInputView: FC<event_typeProps> = observer((props) => {
  ...
  return (
    <Container>
      <Card component={Paper} elevation={5}>
        <CardContent>
          <Box id="event_type_TitleName" sx={{ m: 1 }}>
            <h3>{translate("label:event_typeAddEditView.entityTitle")}</h3>
          </Box>
          <Divider />
          <Grid container direction="row" justifyContent="center" alignItems="center" spacing={1}>
            {columns.map((col) => {
              const id = "id_c_title_event_type_" + col.field;
              if (col.width == null) {
                return (
            ...
            return (
              <>
                <Grid
                  ...
                  sx={style}
                  spacing={1}
                  id="id_event_type_row"
                >
                  {columns.map((col) => {
                    const id = "id_event_type_" + col.field + "_value";
                    if (col.width == null) {
                      return (
                  ...
                  <Grid item display={"flex"} justifyContent={"center"} xs={1}>
                    {storeList.isEdit === false && (
                      <>
                        ...
                        <IconButton
                          id="id_event_typeDeleteButton"
                          name="delete_button"
                          style={{ margin: 0, padding: 0 }}
                          onClick={() => storeList.deleteevent_type(entity.id)}
                        >
                          <DeleteIcon />
                        </IconButton>
                      </>
