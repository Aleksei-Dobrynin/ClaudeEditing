...
import { getApplicationPagination, getApplicationsPaginationForEO } from "api/Application/useGetApplications";
...
class NewStore {
  ...
  loadApplications = async () => {
    try {
      ...
      const response = await getApplicationsPaginationForEO(this.filter);
      if ((response.status === 201 || response.status === 200) && response?.data !== null) {
  ...
  exportApplicationsToExcel = async () => {
    try {
      ...
      const response = await getApplicationsPaginationForEO(excelFilter);
