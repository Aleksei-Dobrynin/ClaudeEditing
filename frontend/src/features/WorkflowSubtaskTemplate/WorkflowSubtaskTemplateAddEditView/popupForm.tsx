...
import WorkflowSubtaskTemplateAddEditBaseView from './base';
...
const PopupForm: FC<PopupFormProps> = observer((props) => {
  ...
  useEffect(() => {
    if (props.openPanel) {
      ...
      store.handleChange({ target: { name: "idWorkflowTaskTemplate", value: props.idWorkflowTaskTemplate } })
    } else {
      store.clearStore()
    }
  }, [props.openPanel])
  ...
  return (
    <Dialog open={props.openPanel} onClose={props.onBtnCancelClick}>
      <DialogTitle>{translate('label:WorkflowSubtaskTemplateAddEditView.entityTitle')}</DialogTitle>
      <DialogContent>
